...
export default function WhatsAppPage() {
  ...
  const checkConnection = async () => {
    try {
      const res = await fetch("/api/plugins/whatsapp/test-connection");
  ...
  const fetchStats = async () => {
    try {
      const [channelsRes, signalsRes, tradesRes] = await Promise.all([
        fetch("/api/plugins/whatsapp/channels"),
        fetch("/api/plugins/whatsapp/signals?status=active&limit=1"),
        fetch("/api/plugins/whatsapp/sniper/trades?status=placed,filled&limit=1"),
      ]);
