...
export default function WhatsAppSettings({ onSessionChange }) {
  ...
  const fetchConfig = async () => {
    try {
      const res = await fetch("/api/plugins/whatsapp/settings");
  ...
  const fetchSession = async () => {
    try {
      const res = await fetch("/api/plugins/whatsapp/session/default/status");
      const data = await res.json();
      setSession(data);
  ...
  const handleCreateSession = async () => {
    ...
    setMessage({ type: "", text: "" });
    try {
      const res = await fetch("/api/plugins/whatsapp/session/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchQrCode = async () => {
    try {
      const res = await fetch("/api/plugins/whatsapp/session/default/qr");
      const data = await res.json();
      if (data.qr_code) {
  ...
  const handleTestConnection = async () => {
    ...
    setMessage({ type: "", text: "" });
    try {
      const res = await fetch("/api/plugins/whatsapp/test-connection", { method: "POST" });
      const data = await res.json();
      if (data.any_ok) {
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                ...
                {session?.db?.status === "ready" && (
                  <button
                    onClick={async () => {
                      await fetch("/api/plugins/whatsapp/session/default/stop", { method: "POST" });
                      fetchSession();
                    }}
