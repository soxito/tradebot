 ...
 *
 * Animation states:
 *   • idle      — subtle breathing, slow blink, gentle sway, occasional head look
 *   • walking   — full bipedal walk cycle: legs/arms swing, torso sways, head bobs
 *   • listening — stops walking, leans forward, antenna perks, mic pulse ring
 *   • thinking  — stops walking, head tilts, orbiting rings, eye flicker
 *   • talking   — stops walking, visor mouth-bars sync to speech energy, head nods
 *
 * Self-contained: creates its own WebGL renderer, disposes on unmount, degrades
...
export default function JarvisRobot({
  ...
  useEffect(() => {
    ...
    const theme = THEMES[styleRef.current]
    ...
    const animate = () => {
      ...
      const en = Math.min(1, Math.max(0, energyRef.current))

      // ── Determine walking vs standing ───────────────────────────────────────
      const isWalking = st === 'idle' || st === 'walking'
      const walkFreq = 2.2
      const walkAmp = isWalking ? 1 : 0
      // Smooth walk amplitude
      const targetWalk = isWalking ? 1 : 0
